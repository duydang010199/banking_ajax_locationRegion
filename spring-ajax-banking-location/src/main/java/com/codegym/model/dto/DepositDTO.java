    ...
    public void validate(@NotNull Object target, @NotNull Errors errors) {
        ...
        if (transactionAmount != null && transactionAmount.length() > 0) {
            if (transactionAmount.length() > 9){
                errors.rejectValue("transactionAmount", "transactionAmount.max", "Số tiền chuyển khoản tối đa là 1.000.000.000 $");
                return;
            }

            if (transactionAmount.length() < 3){
                errors.rejectValue("transactionAmount", "transactionAmount.min.length", "Số tiền chuyển khoản thấp nhất là 100$");
                return;
            }
